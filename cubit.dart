...
import 'package:methpraba/shared/states.dart';
import 'package:bloc/bloc.dart';
import 'package:flutter/material.dart';
import 'package:methpraba/modules/new_tasks_screen.dart';
import 'package:methpraba/modules/done_tasks_screen.dart';
import 'package:methpraba/modules/archived_tasks_screen.dart';
...
class AppCubit extends Cubit<AppStates> {
  ...
  updateToDatabase(
    ...
    await database!.transaction((txn) async {
      txn
          .rawInsert(
              'UPDATE tasks (title, date, time, status) VALUES ("$title","$date","$time")')
          .then((value) {
        getDataBase(database);
        print('$value Updated Successfully');
        emit(AppInsertDatabaseState());
      }).catchError((error) {
        print('Error When updating Table ${error.toString()}');
      });
    });
  }
